...
def runnerr(args, device):
    log_dir = f'/nips_UniImb/log/topology/{args.dataset}'
    ...
    for count in range(args.runs):
        ...
        for epoch in tqdm(range(0, args.epochs)):
            ...
            bias = bias - 0.001 * torch.sgn(topk - args.TopK_2 * 2 * args.batch_size / args.Prototypes)
        ...
        print(Fore.CYAN + f"Run {count + 1}: F1_macro = {F1_macro[count]}, AUROC = {AUROC[count]}, Balanced_accuracy= {Balanced_accuracy[count]}, Accuracy = {Accuracy[count]}")
        ...
        with open(log_filename, 'a') as log_file:
            log_file.write(f"\nRun {count+1}: F1_macro = {F1_macro[count]}, AUROC = {AUROC[count]}, Balanced_accuracy= {Balanced_accuracy[count]}, Accuracy = {Accuracy[count]}\n")
