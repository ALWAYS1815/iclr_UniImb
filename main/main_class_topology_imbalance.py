...
def print_class_distribution(dataset, name=""):
    ...
    for cls, count in sorted(label_count.items()):
        print(f"Class {cls}: {count} samples")
        
        
def runnerr(args, device):
    log_dir = f'/nips_UniImb/log/class_topology/{args.dataset}'
    ...
    for count in range(args.runs):
        ...
        train_mask, val_mask, test_mask, boundary_size = load_split1(load_path=osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'TU', args.dataset), split_mode= args.split_mode)
        ...
        for epoch in tqdm(range(0, args.epochs)):
            loss, n_k = train(model, train_loader, optimizer, args, device, epoch + 1, scheduler, bias)
            ...
            bias = bias - 0.001 * torch.sgn(n_k - args.TopK_2 * 2 * args.batch_size / args.Prototypes)
    ...
    return  F1_macro, AUROC, Balanced_accuracy, Accuracy, F1_micro
...
if __name__ == '__main__':
    ...
    dataset, args.n_feat, args.n_class, _ = get_TUDataset(args.dataset, pre_transform=T.ToSparseTensor(), pos_enc_dim = args.pos_enc)
    ...
    F1_macro, AUROC, Balanced_accuracy, Accuracy, F1_micro = runnerr(args, device)  
